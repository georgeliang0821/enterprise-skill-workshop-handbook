# M6' · Copilot CLI:同一組 .github/ 資產,在終端機執行一次
...
2. 「幫我整理這場會議的紀錄,逐字稿在 lab2-meeting-summary-skill/sample-transcript.txt,會議日期 2026-07-08」→ 比對 VS Code 的輸出(四區塊、日期、Owner)。
...
6. (選)`Shift+Tab` 切到 autopilot 看一眼它的權限提示——不要在這裡開,M7' 會用到雲端那條路。
...
## 決策點 ④(填進 M9 決策表)
哪類工作放 VS Code、哪類放 CLI、哪類派給雲端 agent?
