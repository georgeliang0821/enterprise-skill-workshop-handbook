...
## ③ 步驟(優先包既有工具,不要純手寫)
1. 先用 `datamodel-codegen` 從 OpenAPI spec 產生 Pydantic model——**型別轉換這一段交給決定性工具,不要自己手寫 model**。`--disable-timestamp` 一定要帶,否則每次產出的檔頭時間戳都不同,重跑就無法產生 byte-level 一致的結果:
   ```powershell
   datamodel-codegen --input openapi/meeting-portal-api.yaml --input-file-type openapi --output app/models/schemas.py --disable-timestamp
   ```
...
6. **重跑保護**:若輸出目錄裡目標檔案已存在,逐一檢查每個 function——內容**不是** `TODO` / `NotImplementedError` 骨架(代表已有人填入實作)的,**跳過、不覆蓋**;只新增 spec 新出現的 endpoint、更新純骨架的 function。完成回報裡必須列出「被跳過(保留既有實作)」的 function 清單,讓使用者知道哪些沒被動到。
7. 產出後,實際跑一次 `uvicorn app.main:app --reload`,確認服務能正常啟動(即使呼叫 endpoint 會得到 `NotImplementedError`,那是預期行為,不是錯誤)。
...
- [ ] 若目標檔案已存在:已有實作的 function 都被正確跳過、未被覆蓋,且回報裡列出了被跳過的清單。
