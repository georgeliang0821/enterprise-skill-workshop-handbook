# M2a 漂移實驗 + Lab 2:建立 Meeting Notes Skill

這個資料夾涵蓋兩段:先做 **M2a 撞牆實驗**(15 分鐘,在學任何 SKILL.md 語法之前),再進 **Lab 2** 把實驗裡看到的漂移收斂掉。順序不要顛倒——先親眼看到問題,再學解法。

## M2a · 漂移實驗:同一句 prompt,N 種輸出
...
3. 跑完**不要修、不要重跑**,保留第一次輸出(存成 `baseline-output.md` 之類的檔案,等一下 Lab 2 要拿來對比)。
...
> **給講師**:抽樣優先挑「Due Date 換算」跟「SAP 誤列」——最不需要解釋就能看出會出事。現場漂移不明顯時,備案是投影事先收集的三份歷史輸出。

## Lab 2:建立 Meeting Notes Skill

### 目標

自己動手寫一份 `SKILL.md`,把 M2a 那句「幫我整理這場會議的紀錄」收斂成穩定輸出四個區塊:Meeting Summary / Decisions / Risks / Action Items。

摘要跟待辦合併成同一個 Skill,是因為:權限邊界一樣(都只讀逐字稿、不寫外部系統)、實務上幾乎永遠一起被需要(摘要拿去報告、待辦拿去追蹤)。詳細判斷邏輯見講義 M2b「Scope 怎麼抓」。

### 操作步驟

1. 建立 `.github/skills/meeting-summary/SKILL.md`,對照講義的四區塊骨架**自己填寫**(不要直接複製 `skills/meeting-summary/SKILL.md` 完成版——那份是對照用)。寫的時候,把 M2a 觀察到的每一種漂移對應成一條明確規則:什麼算決議、日期怎麼換算、Owner 用姓名不用職稱、區塊為空也要寫「無」。
2. 存檔後,Chat 輸入 `/skills` 確認被偵測到。
3. **關鍵一步:用跟 M2a 一模一樣的方式再跑一次**——同一份逐字稿(開頭已標註會議日期 2026-07-08)、同一句「幫我整理這場會議的紀錄」,**不要點名 Skill 名稱**,觀察有沒有自動選中。
...
- [ ] 跟 M2a baseline 並排比對,四種漂移(結構/日期/決議判定/Owner)都被收斂
...
> 📌 **留著這三個 Skill 檔案**:Lab 3 做完後,`.github/skills/` 裡會有三個 Skill 並存——M3c 的誤觸發實測要用。
